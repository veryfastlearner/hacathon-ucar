#!/usr/bin/env python3
"""
Fixed GraphRAG Bridge: Supabase → FalkorDB via LlamaIndex
- Fix SSL permission error at the source
- Use modern imports and PropertyGraphIndex
- Semantic bridge via GeminiEmbedding similarity >0.85
"""

import os
# Fix the SSL permission error at the source
...
llm = Gemini(model_name="models/gemini-1.5-flash-preview", api_key=GEMINI_API_KEY)
embed_model = GeminiEmbedding(model_name="models/text-embedding-004", api_key=GEMINI_API_KEY)

# 2. Setup the Graph Store
...
def build_graph_store():
    ...
    # Build the index with documents and graph store
...
def main():
    print("🚀 Connected successfully! Ready to bridge Supabase to FalkorDB.")
    try:
