#!/usr/bin/env python3
"""
Final GraphRAG Bridge - Working implementation
"""
...
# Setup FalkorDB with Redis connection (not bolt)
...
def main():
    print("🚀 Final GraphRAG Bridge")
    print("Note: FalkorDB needs to be running on Redis mode (port 6379)")
    print("Or use Neo4j with bolt://localhost:7687 if you prefer")
    ...
    try:
        ...
        print(f"❌ Error: {e}")
        print("\nTo run FalkorDB in Redis mode:")
        print("docker run -d --name falkordb -p 6379:6379 falkordb/falkordb")
        print("\nOr for Neo4j mode:")
        print("docker run -d --name falkordb -p 7687:7687 -p 7474:7474 falkordb/falkordb")
